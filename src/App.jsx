...
function Tracker({ session }) {
  ...
  const mapConfig = (row) => ({
    livingBudget: Number(row.living_budget),
    totalDays: row.total_days,
    startISO: row.start_date,
    baseSavings: Number(row.base_savings),
  });
  ...
  const load = useCallback(async () => {
    ...
    if (cfg) setConfig(mapConfig(cfg));
      ...
      await supabase.from("config").insert({
        user_id: uid,
        living_budget: DEFAULT_CONFIG.livingBudget,
        total_days: DEFAULT_CONFIG.totalDays,
        start_date: DEFAULT_CONFIG.startISO,
  ...
  const m = useMemo(() => {
    ...
    const projectedSavings =
      config.baseSavings + (config.livingBudget - projectedTotal);
  ...
  const addSpend = async (val) => {
    ...
    const { data, error } = await supabase
      .from("entries")
      .insert({ user_id: uid, spent_on: optimistic.date, amount: n })
      .select()
      .single();
    if (error) {
  ...
  const saveSettings = async () => {
    ...
    const clean = {
      livingBudget:
        parseFloat(String(draft.livingBudget).replace(",", ".")) ||
        DEFAULT_CONFIG.livingBudget,
      totalDays: parseInt(draft.totalDays) || DEFAULT_CONFIG.totalDays,
      baseSavings:
        parseFloat(String(draft.baseSavings).replace(",", ".")) ||
        DEFAULT_CONFIG.baseSavings,
      startISO: draft.startISO || DEFAULT_CONFIG.startISO,
    };
    setConfig(clean);
    await supabase.from("config").upsert({
      user_id: uid,
      living_budget: clean.livingBudget,
      total_days: clean.totalDays,
      start_date: clean.startISO,
      base_savings: clean.baseSavings,
  ...
  return (
    <div style={wrap}>
      ...
      >
        <div>
          ...
          <div style={{ fontSize: 13, marginTop: 3 }}>
            ...
            <b style={{ color: C.blue }}>{m.daysRemaining} дн.</b>
          </div>
        </div>
        <div style={{ display: "flex", gap: 8 }}>
          <button onClick={openSettings} style={iconBtn} aria-label="Настройки">
            ⚙︎
          </button>
          <button
      ...
      {tab === "budget" ? (
        <>
          ...
          <div style={{ ...panel, padding: "22px 20px 20px" }}>
            ...
            <div style={{ fontSize: 13, color: C.muted, marginTop: 4 }}>
              лимит {eur(m.todayAllowance)} = база {eur(m.baselineDaily)}
              {m.carryIn >= 0
                ? ` + перенос ${eur(m.carryIn)}`
                : ` − перерасход ${eur(-m.carryIn)}`}{" "}
          ...
          {/* перенос / сэкономлено за прошлые дни */}
          <div
            style={{
              ...panel,
              marginTop: 12,
              borderColor: m.carryIn >= 0 ? "#1e4d38" : "#4d2320",
            }}
          >
            <div style={label}>
              {m.carryIn >= 0
                ? "Сэкономлено за прошлые дни"
                : "Перерасход за прошлые дни"}
            </div>
            <div
              style={{
                display: "flex",
            ...
            >
              <div
                style={{
                  ...
                  fontWeight: 700,
                  fontSize: 32,
                  color: m.carryIn >= 0 ? C.green : C.red,
                }}
              >
                {signEur(m.carryIn)}
              </div>
              <div style={{ fontSize: 13, color: C.muted }}>
                перенесено в сегодняшний лимит
              </div>
            </div>
            <div
              style={{
            ...
            >
              Если закрыть день сейчас, на завтра перейдёт{" "}
              {signEur(m.carryTomorrow)}.
            </div>
          </div>
          ...
          <div
            style={{
              ...panel,
              marginTop: 12,
              background: "linear-gradient(135deg,#1a2740,#191C22)",
              borderColor: "#243350",
            }}
          >
            <div style={label}>Накопления</div>
            <div style={{ fontSize: 12, color: C.muted, marginTop: 3 }}>
              прогноз к зарплате при текущем темпе
            </div>
            <div
              style={{
                fontFamily: mono,
            ...
            <div style={{ display: "flex", gap: 10, marginTop: 12 }}>
              <MiniStat
                label={`Осталось из ${eur(config.livingBudget)}`}
      ...
      >
        Тикеты на еду считаются отдельно и сюда не входят.
        <br />
        Подушка 350 € — неприкосновенна.
      </div>
        ...
        <div style={overlay} onClick={() => setShowSettings(false)}>
          <div style={modal} onClick={(e) => e.stopPropagation()}>
            ...
            <div style={{ fontSize: 12, color: C.muted, marginBottom: 16 }}>
              После двух недель можно поднять бюджет здесь.
            </div>
            <Field
              label="Живые деньги, €"
              value={draft.livingBudget}
              onChange={(v) => setDraft({ ...draft, livingBudget: v })}
            />
            <Field
              label="Всего дней"
            ...
            />
            <Field
              label="База накоплений, €"
              value={draft.baseSavings}
              onChange={(v) => setDraft({ ...draft, baseSavings: v })}
            />
