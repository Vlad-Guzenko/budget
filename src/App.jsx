...
const daysBetween = (a, b) =>
  Math.round((dateFromISO(b) - dateFromISO(a)) / 86400000);
const addDays = (iso, n) => {
...
export default function App() {
  ...
  const [accent, setAccentState] = useState(() => {
    try {
      return localStorage.getItem("accent") || ACCENTS[0].color;
    } catch {
...
const sumObl = (obl) =>
  (obl || []).reduce((s, o) => s + (parseNum(o.amount) || 0), 0);
...
function Tracker({ session, accent, setAccent }) {
  ...
  const load = useCallback(async () => {
    ...
    if (!per) {
      const ins = await supabase
        ...
        .insert({
          user_id: uid,
          label: DEFAULT_PERIOD.label,
          living_budget: DEFAULT_PERIOD.livingBudget,
          total_days: DEFAULT_PERIOD.totalDays,
          start_date: DEFAULT_PERIOD.startISO,
          status: "active",
        })
        .select()
        .single();
    ...
    setPeriod({
      id: per.id,
      label: per.label,
      livingBudget: Number(per.living_budget),
      totalDays: per.total_days,
      startISO: per.start_date,
      income: Number(per.income || 0),
      obligations: per.obligations || [],
      extras: per.extras || [],
    ...
    setGoals(
      (gl ?? []).map((r) => ({
        id: r.id,
        name: r.name,
        target: r.target_amount != null ? Number(r.target_amount) : null,
        deadline: r.deadline,
  ...
  const m = useMemo(() => {
    ...
    const reservedSavings =
      period.income > 0 ? ownSavings : period.reservedSavings;
    const baseLiving =
      period.income > 0
        ? Math.max(
            0,
            period.income - sumObl(period.obligations) - oneoffSum - ownSavings,
          )
        : period.livingBudget;

    const budgetIncome = incomes
      .filter((i) => i.status === "budget" && i.periodId === period.id)
    ...
    const spentBeforeToday = entries
      .filter((e) => e.date < today)
      .reduce((s, e) => s + e.amount, 0);
  ...
  const addSpend = async (val) => {
    ...
    if (!isFinite(n) || n <= 0 || !period) return;
    setInput("");
    ...
    setEntries((prev) =>
      prev.map((e) =>
        e.id === optimistic.id
          ? { id: data.id, date: data.spent_on, amount: Number(data.amount) }
          : e,
      ),
    );
  };
  ...
  const routeIncome = async (income, target, goalId = null) => {
    ...
    if (target === "budget") {
      await supabase
        .from("incomes")
        .update({ status: "budget", period_id: period.id, goal_id: null })
        .eq("id", income.id);
      setIncomes((prev) =>
        prev.map((i) =>
          i.id === income.id
            ? { ...i, status: "budget", periodId: period.id, goalId: null }
            : i,
        ),
      );
      ...
      await supabase
        .from("incomes")
        .update({ status: "savings", period_id: null, goal_id: goalId })
        .eq("id", income.id);
      const { data } = await supabase
        .from("contributions")
        .insert({
      ...
      setIncomes((prev) =>
        prev.map((i) =>
          i.id === income.id
            ? { ...i, status: "savings", periodId: null, goalId }
            : i,
        ),
      );
      if (data)
      ...
      await supabase
        .from("incomes")
        .update({ status: "pending", period_id: null, goal_id: null })
        .eq("id", income.id);
      setIncomes((prev) =>
        prev.map((i) =>
          i.id === income.id
            ? { ...i, status: "pending", periodId: null, goalId: null }
            : i,
        ),
      );
    }
  ...
  const addGoal = async (name, target, deadline, seed) => {
    const { data } = await supabase
      ...
      .insert({
        ...
        seed: isFinite(parseNum(seed)) ? parseNum(seed) : 0,
        status: "active",
      })
      .select()
      .single();
    if (data)
      setGoals((prev) => [
        ...prev,
        {
          ...
          name: data.name,
          target:
            data.target_amount != null ? Number(data.target_amount) : null,
  ...
  const editGoal = async (goal, patch) => {
    await supabase
      ...
      .eq("id", goal.id);
    setGoals((prev) =>
      prev.map((g) => (g.id === goal.id ? { ...g, ...patch } : g)),
    );
  };
  ...
  const closeMonth = async ({ leftoverMove, leftoverGoal, np }) => {
    ...
    if (leftoverMove > 0 && leftoverGoal) {
      await supabase.from("contributions").insert({
        user_id: uid,
        goal_id: leftoverGoal,
        amount: leftoverMove,
        source: "month_close",
        note: "Остаток периода",
      });
    }
    const inc = parseNum(np.income) || 0;
    const obl = (np.obligations || [])
      .filter((o) => (parseNum(o.amount) || 0) > 0)
      .map((o) => ({
        name: o.name || "Расход",
    ...
    await supabase.from("periods").insert({
      user_id: uid,
      label: np.label || "Новый период",
      living_budget: living,
      total_days: parseInt(np.days) || 30,
      start_date: todayStr(),
      status: "active",
      income: inc,
      obligations: obl,
      reserved_savings: save,
    });
    if (save > 0 && np.startGoal) {
      await supabase.from("contributions").insert({
        user_id: uid,
        goal_id: np.startGoal,
        amount: save,
        source: "period_start",
        note: "Из зарплаты",
      });
  ...
  const saveSettings = async (draft) => {
    ...
    const reserved =
      inc > 0
        ? goals.reduce((s, g) => s + goalBalance(g, contribs), 0)
        : period.reservedSavings;
    const living =
      inc > 0
        ? Math.max(0, inc - sumObl(obl) - sumObl(period.extras) - reserved)
        : period.livingBudget;
    const patch = {
      label: draft.label || period.label,
      totalDays: parseInt(draft.totalDays) || period.totalDays,
      startISO: draft.startISO || period.startISO,
      income: inc,
      obligations: obl,
    ...
    await supabase
      ...
      .update({
        living_budget: living,
        total_days: patch.totalDays,
        start_date: patch.startISO,
        label: patch.label,
        income: inc,
        obligations: obl,
    ...
    <div style={wrap}>
      ...
      {routeFor && (
        <GoalPicker
          ...
          contribs={contribs}
          onPick={(gid) => {
            routeIncome(routeFor, "savings", gid);
            setRouteFor(null);
          }}
...
function BudgetOverview({
  ...
  return (
    <>
      ...
      >
        <div style={{ ...label, alignSelf: "flex-start" }}>
          Осталось на сегодня
        </div>
        <div
          style={{
        ...
        >
          <svg width="184" height="184" viewBox="0 0 184 184">
            ...
            <circle
              ...
              strokeDashoffset={
                2 * Math.PI * 82 * (1 - Math.min(Math.max(m.ratio, 0), 1))
              }
          ...
          >
            <div
              style={{
                ...
                fontSize: 34,
                letterSpacing: -0.5,
                color: m.leftToday < 0 ? C.red : C.text,
                whiteSpace: "nowrap",
              }}
            >
              {eur(m.leftToday)}
        ...
        {m.carryIn > 0.005 && (
          ...
          >
            ...
            <span style={{ fontSize: 12.5, color: "#8FBFA8", lineHeight: 1.3 }}>
              сэкономлено — добавлено к лимиту
            </span>
          </div>
        )}
        {m.carryIn < -0.005 && (
          ...
          >
            ...
            <span style={{ fontSize: 12.5, color: "#C39A9A", lineHeight: 1.3 }}>
              перерасход — вычтено из лимита
            </span>
          </div>
        )}
        ...
        <div style={{ display: "flex", width: "100%", marginTop: 16, gap: 8 }}>
          {[
            ["база", eur(m.baselineDaily)],
            ["потрачено", eur(m.spentToday)],
            ...(m.daysRemaining > 1
              ? [["завтра", signEur(m.carryTomorrow)]]
              : []),
          ].map(([k, v], i) => (
            <div
              key={i}
      ...
      {m.budgetIncome > 0.005 && period.income <= 0.005 && (
        ...
        >
          <span style={{ fontFamily: mono, fontWeight: 700, color: C.green }}>
            +{eur(m.budgetIncome)}
          </span>
          <span style={{ fontSize: 12.5, color: C.muted }}>
            поступлений добавлено в бюджет месяца
          </span>
        </div>
      )}
      ...
      <div style={{ ...panel, marginTop: 12 }}>
        ...
        <div style={{ fontSize: 12, color: C.muted, marginTop: 3 }}>
          это можно будет отложить в копилку
        </div>
        <div
          style={{
            fontFamily: mono,
        ...
        <div style={{ display: "flex", gap: 10, marginTop: 12 }}>
          <MiniStat
            label={`Осталось из ${eur(m.effectiveLiving)}`}
            value={eur(m.remainingBudget)}
            color={m.remainingBudget < 0 ? C.red : C.text}
          />
          <MiniStat
            label="Прогноз трат"
            value={eur(m.projectedTotal)}
            color={C.text}
          />
        </div>
        <div style={{ marginTop: 12, fontSize: 12.5, color: C.muted }}>
          Всего в копилках:{" "}
          <b style={{ color: C.text, fontFamily: mono }}>{eur(savingsTotal)}</b>
        </div>
      </div>
      ...
      >
        Тикеты на еду считаются отдельно.
        <br />
        Дневной лимит фиксирован; поступления — отдельно, во вкладке «Доходы».
      </div>
    </>
  );
}
...
function IncomeView({
  ...
  const inBudget = incomes
    .filter((i) => i.status === "budget")
    .reduce((s, i) => s + i.amount, 0);
  const inSavings = incomes
    .filter((i) => i.status === "savings")
    .reduce((s, i) => s + i.amount, 0);
  const pendingSum = pending.reduce((s, i) => s + i.amount, 0);

  const submit = async () => {
    await addIncome(amount, date, source);
    setAmount("");
    setSource("");
    setDate(todayStr());
    setAdding(false);
  ...
  return (
    <>
      <div style={panel}>
        ...
        <div style={{ fontSize: 12, color: C.muted, marginTop: 3 }}>
          подарки, бонусы, разовые деньги — не влияют на дневной лимит, пока ты
          сам не решишь
        </div>
        <div style={{ display: "flex", gap: 10, marginTop: 14 }}>
          <MiniStat
            ...
            color={pendingSum > 0 ? C.amber : C.muted}
          />
      ...
      {!adding ? (
        ...
        <div style={{ ...panel, marginTop: 12 }}>
          ...
          <input
            type="text"
            placeholder="Источник (необязательно): подарок, бонус…"
            value={source}
          ...
          <div style={{ display: "flex", gap: 8, marginTop: 12 }}>
            ...
            <button
              onClick={submit}
              style={{ ...primaryBtn, flex: 1, padding: "12px" }}
            >
              Добавить
            </button>
          </div>
      ...
      >
        ...
        {incomes.map((i) => {
          const badge =
            i.status === "budget"
              ? { t: "в бюджете", c: C.blue }
              : i.status === "savings"
                ? { t: "в копилке · " + goalName(i.goalId), c: C.green }
          ...
          return (
            <div key={i.id} style={panel}>
              ...
              >
                ...
                {i.status !== "savings" && (
                  <button onClick={() => sendToSavings(i)} style={miniBtn}>
                    В копилку
                  </button>
                )}
                {i.status !== "pending" && (
                  <button
                    onClick={() => routeIncome(i, "pending")}
                    style={miniBtn}
                  >
                    Вернуть
                  </button>
    ...
    <>
      ...
      {!creating ? (
        ...
        <div style={{ ...panel, marginTop: 12 }}>
          ...
          <input
            type="text"
            placeholder="Название (Мотоцикл, Отпуск…)"
            value={nm}
          ...
          <input
            type="number"
            inputMode="decimal"
            placeholder="Уже накоплено, € (стартовая сумма)"
            value={sd}
...
function GoalCard({
  ...
  const [enm, setEnm] = useState(goal.name);
  const [etg, setEtg] = useState(goal.target ?? "");
  const [edl, setEdl] = useState(goal.deadline ?? "");

  const list = contribs.filter((c) => c.goalId === goal.id);
  const bal = goalBalance(goal, contribs);
  const pct = goal.target
    ? Math.min(100, Math.max(0, (bal / goal.target) * 100))
    : null;
  ...
  const doAdd = async () => {
    await onContribute(goal.id, amt, null, "manual");
    setAmt("");
    setMode(null);
  };
  const doSub = async () => {
    const n = parseNum(amt);
    if (isFinite(n) && n > 0) await onContribute(goal.id, -n, null, "manual");
    setAmt("");
    setMode(null);
  };
  const doEdit = async () => {
    await onEdit(goal, {
      name: enm || goal.name,
      target: isFinite(parseNum(etg)) ? parseNum(etg) : null,
      deadline: edl || null,
    });
    setMode(null);
  };
  ...
  const srcLabel = {
    income: "поступление",
    month_close: "остаток периода",
    period_start: "из зарплаты",
    manual: "вручную",
  };
      ...
      {mode === "edit" ? (
        <div style={{ marginTop: 10 }}>
          ...
          <input
            ...
            placeholder="Цель, € (пусто = без цели)"
            style={{ ...inputStyle, marginTop: 8 }}
          />
          <input
            type="date"
            value={edl || ""}
            onChange={(e) => setEdl(e.target.value)}
            style={{ ...inputStyle, marginTop: 8 }}
          />
          <div style={{ display: "flex", gap: 8, marginTop: 10 }}>
            ...
            <div style={{ flex: 1 }} />
            <button onClick={() => setMode(null)} style={miniBtn}>
              Отмена
            </button>
            <button
              onClick={doEdit}
          ...
          {goal.target != null && (
            <>
              <div style={{ fontSize: 12, color: C.muted, marginTop: 2 }}>
                из {eur(goal.target)} · осталось{" "}
                {eur(Math.max(0, goal.target - bal))}
              </div>
              <div
                style={{
                  height: 8,
              ...
              >
                <div
                  style={{
                    height: "100%",
                    width: pct + "%",
                    background: pct >= 100 ? C.green : "var(--accent, #3B82F6)",
                    transition: "width .5s",
                  }}
                />
              </div>
                  ...
                  <div>
                    <div
                      style={{
                        fontFamily: mono,
                        fontSize: 15,
                        color: c.amount < 0 ? C.red : C.green,
                      }}
                    >
                      {signEur(c.amount)}
                    </div>
      ...
      <div style={panel}>
            ...
            {data.map((d, k) => {
              ...
              return (
                <g key={d.date}>
                  <rect
                    x={x}
                    y={d.spent === 0 ? padT + innerH - 2 : y}
                    width={bw}
                    height={d.spent === 0 ? 2 : Math.max(h, 1)}
                    rx="2"
                    fill={col}
        ...
        <div style={{ display: "flex", gap: 10, marginTop: 12 }}>
          ...
          <MiniStat
            ...
            value={eur(avg)}
            color={avg > target ? C.amber : C.green}
          />
        </div>
      </div>
      <div style={{ ...panel, marginTop: 12 }}>
...
function HistoryView({ period, entries, m, onRemove }) {
  ...
  const weeks = useMemo(() => {
    ...
    const days = Object.entries(byDate).map(([date, list]) => ({
      date,
      weekIdx: Math.floor(daysBetween(period.startISO, date) / 7),
      ...
      {weeks.map((w) => {
        ...
        return (
          <div key={w.idx} style={panel}>
            ...
            >
              <span>
                {prettyDate(wStart)} – {prettyDate(wEnd)}
              </span>
                  ...
                  >
                    ...
                    <span
                      style={{
                        ...
                        fontSize: 15,
                        color:
                          d.total === 0 ? C.muted : dOver ? C.amber : C.green,
                      }}
                    >
                      {eur(d.total)}
                    </span>
                    ...
                    <div style={{ paddingBottom: 8 }}>
                      {d.list.map((e) => (
                        ...
                        >
                          ...
                          <button onClick={() => onRemove(e.id)} style={delBtn}>
                            удалить
                          </button>
                        </div>
                      ))}
...
function SettingsModal({
  ...
  const living =
    inc > 0 ? Math.max(0, inc - oblSum - savingsTotal) : period.livingBudget;
    ...
    <div style={overlay} onClick={onClose}>
      ...
      >
        ...
        <div style={{ fontSize: 12, color: C.muted, marginBottom: 16 }}>
          Заполни расклад — «на жизнь» посчитается само.
        </div>
          ...
          {ACCENTS.map((a) => (
            <button
              ...
              style={{
                ...
                boxShadow: accent === a.color ? `0 0 0 2px ${a.color}` : "none",
                cursor: "pointer",
              }}
            />
          ))}
        ...
        />
        <Field
          label="Пришло за период, € (баланс/зарплата)"
          value={draft.income}
        ...
        <Field
          ...
          onChange={(v) => setDraft({ ...draft, totalDays: v })}
        />
        <Field
          label="Старт (ГГГГ-ММ-ДД)"
          value={draft.startISO}
          type="text"
          onChange={(v) => setDraft({ ...draft, startISO: v })}
        />

        {inc > 0 && (
          <div
            style={{ ...panel, padding: 14, marginTop: 4, marginBottom: 12 }}
          >
            <div
              style={{
                display: "flex",
            ...
            >
              ...
              <span style={{ fontFamily: mono, color: C.green }}>
                −{eur(savingsTotal)}
              </span>
            </div>
  ...
  return (
    <div style={overlay} onClick={onClose}>
      <div style={modal} onClick={(e) => e.stopPropagation()}>
        ...
        <button
          onClick={onClose}
          style={{ ...chip, width: "100%", padding: "12px", marginTop: 12 }}
        >
          Отмена
        </button>
      </div>
    ...
    <div style={overlay} onClick={onClose}>
      ...
      >
        ...
        <div style={{ fontSize: 12, color: C.muted, marginBottom: 14 }}>
          Сначала — остаток текущего, потом — расклад нового месяца.
        </div>

        <div style={{ ...panel, padding: 14, marginBottom: 14 }}>
          <div style={{ fontSize: 12, color: C.muted }}>
            Остаток текущего периода
          </div>
          <div
            style={{
              fontFamily: mono,
              fontWeight: 700,
              fontSize: 24,
              color: leftover > 0 ? C.green : C.muted,
              marginTop: 4,
            }}
          >
            {eur(leftover)}
          ...
          {goals.length > 0 && leftover > 0 && (
            <div style={{ marginTop: 10 }}>
              <Field
                label="Отложить остаток в копилку, €"
                value={move}
                onChange={setMove}
              />
              <select
                value={leftoverGoal}
                onChange={(e) => setLeftoverGoal(e.target.value)}
                style={inputStyle}
              >
                {goals.map((g) => (
                  <option key={g.id} value={g.id}>
                    {g.name} · {eur(goalBalance(g, contribs))}
                  </option>
                ))}
              </select>
            </div>
        ...
        {goals.length > 0 && (
          <div style={{ marginTop: 4 }}>
            <Field
              label="Сразу в копилку, € (необязательно)"
              value={startSavings}
              onChange={setStartSavings}
            />
            {save > 0 && (
              <select
                value={startGoal}
        ...
        <div style={{ ...panel, padding: 14, marginBottom: 12 }}>
          ...
          >
            <span style={{ color: C.muted }}>Зарплата</span>
            <span style={{ fontFamily: mono }}>{eur(inc)}</span>
          </div>
          <div
            style={{
              display: "flex",
          ...
          >
            <span style={{ color: C.muted }}>− Обязательные</span>
            <span style={{ fontFamily: mono, color: C.red }}>
              −{eur(oblSum)}
            </span>
          </div>
          {save > 0 && (
            ...
            >
              <span style={{ color: C.muted }}>− В копилку</span>
              <span style={{ fontFamily: mono, color: C.green }}>
                −{eur(save)}
              </span>
            </div>
          ...
          >
            ≈ {eur(living / (parseInt(days) || 30))}/день
          </div>
        </div>
...
function BreakdownCard({
  ...
  const Row = ({ k, v, c, sign }) => (
    ...
    >
      ...
      <span style={{ fontFamily: mono, color: c || C.text }}>
        {sign || ""}
        {eur(v)}
      </span>
    </div>
  ...
  return (
    <div style={{ ...panel, marginTop: 12 }}>
      ...
      <div style={{ marginTop: 8 }}>
        ...
        {budgetIncome > 0.005 && (
          <Row k="Поступления в бюджет" v={budgetIncome} c={C.green} sign="+" />
        )}
        <button
          onClick={() => setOpen(!open)}
        ...
        >
          ...
          >
            <span style={{ color: C.muted }}>
              Обязательные {obl.length ? `(${obl.length}) ▾` : ""}
            </span>
            <span style={{ fontFamily: mono, color: C.red }}>
              −{eur(oblSum)}
            </span>
        ...
        {ownSavings > 0.005 && (
          <Row k="Отложено в копилку" v={ownSavings} c={C.green} sign="−" />
        )}
        <div style={{ height: 1, background: C.border, margin: "8px 0" }} />
        <div
          style={{
        ...
        >
          ≈ {eur(daily)}/день на {period.totalDays} дн.
        </div>
        {extraSavings > 0.005 && (
          <div
            style={{
          ...
          >
            В копилке есть ещё {eur(extraSavings)} сторонних денег (через
            «Доходы») — они не из этих {eur(income)} и на дневной лимит не
            влияют.
          </div>
        )}
      </div>
...
function OneoffCard({ extras, addExtra, removeExtra }) {
  const [adding, setAdding] = useState(false);
  const [name, setName] = useState("");
  const [amount, setAmount] = useState("");
  const [date, setDate] = useState(todayStr());
  const total = sumObl(extras);
  const sorted = [...extras.map((e, i) => ({ ...e, i }))].sort((a, b) =>
    a.date < b.date ? 1 : -1,
  );

  const submit = async () => {
    await addExtra(name, amount, date);
    setName("");
    setAmount("");
    setDate(todayStr());
    setAdding(false);
  ...
  return (
    <div style={{ ...panel, marginTop: 12 }}>
      ...
      >
        ...
        {total > 0.005 && (
          <div style={{ fontFamily: mono, fontSize: 15, color: C.red }}>
            −{eur(total)}
          </div>
        )}
      </div>
      <div style={{ fontSize: 12, color: C.muted, marginTop: 3 }}>
        крупные разовые траты (родители, покупка…) — не входят в дневной лимит
      </div>
          ...
          {sorted.map((e) => (
            ...
            >
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                <span style={{ fontFamily: mono, fontSize: 15 }}>
                  {eur(parseNum(e.amount) || 0)}
                </span>
                <button onClick={() => removeExtra(e.i)} style={delBtn}>
                  удалить
                </button>
              </div>
            </div>
      ...
      {!adding ? (
        <button
          onClick={() => setAdding(true)}
          style={{ ...miniBtn, width: "100%", padding: "11px", marginTop: 12 }}
        >
          + Разовый расход
        </button>
      ) : (
        <div style={{ marginTop: 12 }}>
          <input
            type="text"
            placeholder="На что (Родители, ноутбук…)"
            value={name}
          ...
          <div style={{ display: "flex", gap: 8, marginTop: 8 }}>
            <button
              ...
              style={{ ...chip, flex: 1, padding: "11px" }}
            >
              Отмена
            </button>
            <button
              onClick={submit}
              style={{ ...primaryBtn, flex: 1, padding: "11px" }}
            >
              Добавить
            </button>
          </div>
...
function ObligationList({ items, onChange }) {
  ...
  return (
    <div>
      ...
      <button
        onClick={add}
        style={{ ...miniBtn, width: "100%", padding: "10px", marginTop: 2 }}
      >
        + строка
      </button>
    </div>
